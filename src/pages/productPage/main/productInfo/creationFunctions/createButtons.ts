...
import handleButtonClick from "@pages/productPage/main/productInfo/creationFunctions/handleButtonClick";
    ...
    const label = createElement(Tag.LABEL, {
        ...
        onclick: () => {
            if (!label.classList.contains(styles.attributeActive)) {
                handleButtonClick(productData, attribute.name, label, productID);
            }
        },
    });
